...
CANVAS = "1280x720"   # --canvas 可覆盖（默认前端契约 1280x720；对比实验 1600x900）


def _canvas_wh() -> tuple[str, str]:
    w, _, h = CANVAS.partition("x")
    return (w.strip() or "1280", h.strip() or "720")
...
def _build_prompt(prompt_template: str, slice_obj: dict, mode: str, extra: str = "") -> str:
    w, h = _canvas_wh()
    prompt_template = prompt_template.replace("{{CANVAS_W}}", w).replace("{{CANVAS_H}}", h)
...
def run(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mode", choices=["no_template", "template"], default=None,
                        help="no_template / template；省略时自动从 deck 切片判定")
    parser.add_argument("--prompt", "--gen-prompt", dest="prompt", required=True,
                        help="prompt 模板 md 绝对路径（--gen-prompt 为兼容别名，防与 review_batch 参数名混用）")
    parser.add_argument("--concurrency", type=int, default=DEFAULT_CONCURRENCY)
    parser.add_argument("--canvas", default="1280x720",
                        help="画布尺寸 WxH。默认 1280x720（前端契约）；对比实验可传 1600x900，会同步注入 prompt 与 lint。")
    ...
    if args.canvas and "x" in args.canvas:
        CANVAS = args.canvas.lower().strip()
    ...
    # --mode 省略时从首个切片自动判定（template 切片含 template_map_page/template_html_path）
    mode = args.mode
    if mode is None:
        try:
            probe = json.loads(slice_paths[0].read_text(encoding="utf-8"))
            mode = "template" if (probe.get("template_map_page") or probe.get("template_html_path")) else "no_template"
        except Exception:  # noqa: BLE001
            mode = "no_template"
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.concurrency)) as pool:
        details = list(
            pool.map(
                lambda p: _process_page(p, mode, prompt_template, str(deck_dir)),
                slice_paths,
            )
        )
