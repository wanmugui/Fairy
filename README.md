...
Fairy 是一个本地优先的通用 Agent 工作台：类似 dsh，能够控制电脑、执行命令、调用工具、流式对话，并把会话、工具调用与 Token 用量统一管理。Agent 使用本地 Go 引擎，不依赖外部 agent 服务。

## 项目目标

- 构建一个类似 dsh 的本地 Agent，可以接管电脑完成命令与任务。
- 支持工具调用、多模型切换、流式输出与本地会话持久化。
- 内置 Skill 注册与插件自动搜索：Agent 能按需自动发现、检索并接入可用的技能/插件，而不是只使用写死的工具集。
- Fairy 的带语音版本前端就是 Agent 的统一前端：文本对话与实时语音共用同一套界面和会话。
